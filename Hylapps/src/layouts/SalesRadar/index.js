...
const fetchVesselIMOValues = async (userId) => {
  ...
  //         return false;
  //       });
  //  // Step 2: Filter vessels based on IMO values from salesData
  //  const imoValues = salesData.map((sale) => sale.IMO); // Get IMO values from salesData state
  //  const filteredByImoData = filteredData.filter((vessel) => imoValues.includes(vessel.AIS.IMO));

  //  // Step 3: Map filtered data to formatted data structure
  //  const formattedData = filteredByImoData.map((vessel) => ({
...
useEffect(() => {
  ...
  const filteredByImoData = filteredData.filter((vessel) => imoValues.includes(vessel.AIS.IMO));
      ...
      const transformedData = filteredByImoData.map((vessel) => ({
